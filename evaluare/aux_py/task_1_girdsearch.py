...
# cod (intermediat, nefinisat) pentru un gridsearch pentru task ul 1
...
def solve_task_1_p(img, identify_digits = False, show = False, empty_size = 0.7, empty_err = 0.003, binthr = 120):

    l = img.shape[0]
    chunk_l = l // 9

    empty = np.full((int(chunk_l * empty_size), int(chunk_l * empty_size)), np.uint8(255))
        
    def _is_empty(ch):

        # checks whether there is a contiguous "only-white" portion in the iamge
        # of (3/4, 2/4) of the chunk size
        # with a small error margin determined empirically on train data

        TOLERATED_ERR = empty_err
        ...
        for i in range(9):
            for j in range(9):
                ...
                chunk = img[i * chunk_l + off: (i + 1) * chunk_l + off, 
                            j * chunk_l: (j + 1) * chunk_l, :]
                chunk = cv.cvtColor(chunk, cv.COLOR_RGB2GRAY)
                _, chunk = cv.threshold(chunk, binthr, 255, cv.THRESH_BINARY)
...
def task_1_gridsearch():
    ...
    f = open("gridsearch_2.txt", "w+")

    hyperparam_val =    {
                        "empty_size": [0.66, 0.68, 0.7, 0.72, 0.73, 0.76], 
                        "empty_err": [0.001, 0.003, 0.004, 0.007, 0.01], 
                        "binthr": [110, 115, 117, 120, 125, 130], 
                        }
    ...
    for i in range(1, IMG_CNT + 1):
        ...
        imgs.append([get_center(i), None, None, None, None])
        ...
        imgs[i][3] = f_org.read().split()
        imgs[i][4] = f_org_d.read().split()
    ...
    for hyperparams in _get_hyperparam_seq(hyperparam_names):
        ...
        for i in range(1, IMG_CNT + 1):
            ...
            imgs[i][1] = solve_task_1_p(imgs[i][0], False, False, **hyperparams)
            imgs[i][2] = solve_task_1_p(imgs[i][0], True, False, **hyperparams)

            for j in range(9):

                imgs[i][1][j] = ''.join(imgs[i][1][j])
                imgs[i][2][j] = ''.join(imgs[i][2][j])
            
            if _cmp_m(imgs[i][1], imgs[i][3]):
                ok += 1

            if _cmp_m(imgs[i][2], imgs[i][4]):
                ok_digit += 1

        if ok == 20 and ok_digit >= 18:

            print(f"{hyperparams} - ok: {ok} / 20")
            print(f"{hyperparams} - ok_d: {ok_digit} / 20") 

            print(f"{hyperparams} - ok: {ok} / 20", file = f)
            print(f"{hyperparams} - ok_d: {ok_digit} / 20", file = f)         
